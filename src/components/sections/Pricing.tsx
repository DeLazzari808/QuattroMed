        ...
        <div className="text-left mb-10 lg:mb-16">
          <AnimatedSection>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2 sm:gap-3 mt-8 sm:mt-10">
              <span
                className={`text-sm sm:text-base font-semibold transition-all duration-300 font-modelica ${!isAnnual ? 'text-white' : 'text-white/40'
                  }`}
              >
                Mensal
              </span>
              ...
              >
                <motion.div
                  ...
                  className={`w-5 sm:w-6 h-5 sm:h-6 rounded-full shadow-md transition-colors duration-300 ${isAnnual ? 'bg-white' : 'bg-white'
                    }`}
                />
              </button>
            ...
            <AnimatedSection key={idx} delay={idx * 0.1} className="flex-1">
              ...
              >
                ...
                <div className="flex flex-col h-full p-5 sm:p-6 lg:p-8 pt-8 sm:pt-10">
                  ...
                  {/* Title */}
                  <h3
                    className={`text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-2 font-modelica ${plan.highlight ? 'text-white' : 'text-teal-700'
                      }`}
                  >
                    {plan.name}
                  </h3>
                  ...
                  <div className="mb-2 sm:mb-3">
                    <div className="flex items-baseline gap-0.5 font-modelica">
                      ...
                      <span
                        className={`text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight ${plan.highlight ? 'text-white' : 'text-neutral-900'
                          }`}
                      >
                        {plan.price}
                      </span>
                  ...
                  {/* Description */}
                  <p
                    className={`text-[11px] sm:text-xs lg:text-sm mb-4 sm:mb-5 leading-relaxed font-modelica ${plan.highlight ? 'text-white/90' : 'text-neutral-600'
                      }`}
                  >
                    {plan.description}
                  </p>
                  ...
                  <ul className="space-y-2 sm:space-y-3 mb-5 sm:mb-6 flex-grow">
                    {plan.features.map((feature, fIdx) => (
                      <li key={fIdx} className="flex items-start gap-2">
                        <span
                          className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${plan.highlight ? 'bg-white' : 'bg-teal-600'
                            }`}
                        />
                        <span
                          className={`text-[11px] sm:text-xs leading-relaxed font-modelica ${plan.highlight ? 'text-white' : 'text-neutral-700'
                            }`}
                        >
                          {feature}
                        </span>
