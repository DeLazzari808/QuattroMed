    ...
    <section className="py-24 lg:py-32 relative overflow-hidden" style={{ backgroundColor: '#f8fafb' }}>
      ...
      <Container>
        <div className="text-center mb-16 lg:mb-20">
          ...
          <motion.h2 
            ...
            className="text-3xl md:text-5xl lg:text-6xl font-bold leading-tight font-modelica"
            style={{ color: '#003B46' }}
          >
      ...
      <div className="relative flex overflow-hidden py-10">
        <motion.div 
          ...
          className="flex gap-6 lg:gap-10 whitespace-nowrap"
        >
          {infiniteItems.map((t, idx) => (
            <div 
              key={idx}
              className="w-[350px] lg:w-[450px] flex-shrink-0 bg-white p-8 lg:p-10 rounded-2xl lg:rounded-3xl border transition-all duration-500 hover:shadow-xl group"
              style={{ borderColor: 'rgba(0, 59, 70, 0.1)' }}
            >
              <div className="flex items-center gap-4 mb-6 lg:mb-8">
                <div className="w-12 h-12 lg:w-14 lg:h-14 rounded-full overflow-hidden border" style={{ borderColor: 'rgba(32, 118, 129, 0.2)' }}>
                  <img src={t.image} alt={t.name} className="w-full h-full object-cover group-hover:scale-105 transition-all duration-500" />
                </div>
                <div>
                  <h4 className="font-bold text-lg font-modelica" style={{ color: '#003B46' }}>{t.name}</h4>
                  <p className="text-[10px] lg:text-xs font-bold uppercase tracking-widest font-modelica" style={{ color: '#207681' }}>{t.location}</p>
                </div>
                <div className="ml-auto">
                  <img src="/images/ICONE-C.png" alt="Icon" className="w-10 h-10 object-contain" />
                </div>
              </div>

              <Quote className="mb-4 opacity-10" size={32} style={{ color: '#003B46' }} />
              <p className="text-neutral-600 font-medium whitespace-normal leading-relaxed text-sm lg:text-base font-modelica">
                "{t.text}"
              </p>
            </div>
          ))}
        ...
        {/* Faded edges */}
        <div className="absolute inset-y-0 left-0 w-32 lg:w-64 bg-gradient-to-r from-[#f8fafb] to-transparent z-10"></div>
        <div className="absolute inset-y-0 right-0 w-32 lg:w-64 bg-gradient-to-l from-[#f8fafb] to-transparent z-10"></div>
      </div>
    </section>
  );
};
